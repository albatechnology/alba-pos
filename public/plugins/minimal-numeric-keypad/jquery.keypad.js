    ...
    $.fn[pluginName] = function(options) {
      ...
      return this.each(function() {
        ...
        var numbers = Array.apply(null, Array(9)).map(function (_, i) {
          return $(options.buttonTemplate).html(i+1).addClass('number').attr('type', 'button');
        });
        numbers.push($(options.buttonTemplate).html(options.deleteButtonText).addClass(options.deleteButtonClass).attr('type', 'button'));
        numbers.push($(options.buttonTemplate).html("0").addClass('number').attr('type', 'button'));
        numbers.push($(options.buttonTemplate).html(options.submitButtonText).attr('type', 'button'));
