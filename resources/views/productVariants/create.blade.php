                ...
                <div class="row">
                    <div class="col-12">
                        <div class="card card-primary">
                            <form method="post" action="{{ route('product-variants.store') }}" class="form-loading">
                                @csrf
                                <div class="card-body">
                                    ...
                                    {{-- <div class="form-group">
                                        <label class="required">Selection Type</label>
                                        <div>
                                            @foreach (\App\Enums\ProductVariantType::getInstances() as $productVariant)
                                                <div class="form-check form-check-inline">
                                                    <input name="option" type="radio" value="{{ $productVariant->value }}" class="form-check-input radio-inline @error('option') is-invalid @enderror">
                                                    <label class="form-check-label" for="check_add">{{ $productVariant->description }}</label>
                                                </div>
                                            @endforeach
                                        </div>
                                        @error('option')
