...
<body class="hold-transition sidebar-mini layout-fixed">
    ...
    <img class="animation__shake" src="dist/img/AdminLTELogo.png" alt="AdminLTELogo" height="60" width="60">
                ...
                <div class="user-panel mt-3 pb-3 mb-3 d-flex">
                    ...
                    <div class="info ">
                        <a href="{{ route('profiles.index') }}"
                            class="d-block stretched-link">{{ auth()->user()->name }}</a>
                    </div>
                </div>
                <div class="user-panel mt-3 pb-3 mb-3 d-flex">
                    <form id="tenant-selector" action="{{ url('tenants/set-active') }}" method="POST"
                        class="w-100">
                        @csrf
                        ...
                        <select name="company_id" class="form-control mb-1"
                            ...
                            @foreach (tenancy()->getMyAllCompanies() as $company)
                                <option value="{{ $company->id }}"
                                    {{ tenancy()->activeCompanyIs($company) ? 'selected' : '' }}>{{ $company->name }}
                                </option>
                            @endforeach
                        </select>
                        <select name="tenant_id" class="form-control"
                            ...
                            @foreach (tenancy()->getTenants() as $tenant)
                                <option value="{{ $tenant->id }}"
                                    {{ tenancy()->activeTenantIs($tenant) ? 'selected' : '' }}>{{ $tenant->name }}
                                </option>
                            @endforeach
                        </select>
                    </form>
                ...
                <nav class="mt-2">
                    <ul class="nav nav-pills nav-sidebar flex-column" data-widget="treeview" role="menu"
                            ...
                            <li class="nav-item">
                                <a href="{{ url('/') }}"
                                    class="nav-link {{ request()->is('/') ? 'active' : '' }}"><i
                                        class="nav-icon fa fa-tachometer-alt"></i>
                                    <p>Dashboard</p>
                                </a>
                            </li>
                        @endcan
                        @can('cashier_access')
                            <li class="nav-item">
                                <a href="{{ url('cashier') }}"
                                    class="nav-link {{ request()->is('cashier') ? 'active' : '' }}"><i
                                        class="nav-icon fa fa-cash-register"></i>
                                    <p>Cashier</p>
                                </a>
                            </li>
                        @endcan
                        @foreach (\App\Services\MenuService::menu() as $menu)
                            @can($menu->permission)
                                ...
                                @else
                                    <li
                                        class="nav-item {{ request()->is($menu->submenus[0]->url . '*') ? 'menu-open' : '' }}">
        ...
        <footer class="main-footer">
            <strong>Copyright &copy; <?php echo date('Y'); ?> <a href="https://albatech.id">Albatech</a>.</strong> All
            rights reserved.
        </footer>
