...
<body>
    <table style="width: 100%; border: 0; margin-top: 12px; margin-bottom: 24px;">
        <tr>
            <td style="text-align: left; padding: 0;">
                <img src="{{ asset('images/pempek-warisan.jpg') }}" class="logo" alt="Logo" />
            </td>
                            ...
                            <table>
                                <tr>
                                    ...
                                    <td style="text-align: left; border: 0; padding: 0;">:
                                        {{-- {{ \Carbon\Carbon::parse($data['order']->created_at)->toDayDateTimeString() }} --}}
                                    </td>
                                </tr>
                                <tr>
                                    <td style="text-align: left; border: 0; padding: 0; font-weight: bold;" width="80">
    ...
    <div>
        ...
        <div>
            {{-- {{ \App\Enums\PersonTitle::fromValue($data['order']->customer->title)->description .' ' .$data['order']->customer->getFullNameAttribute() }} --}}
        </div>
        {{-- <div>{{ $data['order']->shipping_address?->city }}</div>
        <div>{{ $data['order']->shipping_address?->province }}</div>
        <div>{{ $data['order']->shipping_address?->country }}</div>
        <div>{{ $data['order']->shipping_address?->postcode }}</div>
        <div>{{ $data['order']->shipping_address?->phone }}</div> --}}
    </div>
    ...
    <table class="table items-table">
        ...
        {{-- <tr>
            ...
            <td class="info-row-right" style="border-top: 1px solid #313132;">
                {{ rupiah($data['order']->packing_fee) }}
            </td>
        </tr>
        <tr>
            <td colspan="6" class="info-row-left">Shipping Fee</td>
            <td class="info-row-right">
                {{ rupiah($data['order']->shipping_fee) }}
            </td>
        </tr> --}}
        {{-- <tr>
            <td colspan="6" class="info-row-left">Sub Total</td>
            <td class="info-row-right">
                {{ rupiah($data['order']->original_price + $data['order']->packing_fee + $data['order']->shipping_fee) }}
            </td>
        </tr>
        <tr>
        ...
        <tr style="margin-bottom: 10px;">
            ...
            <td class="info-row-right" style="border-bottom: 1px solid #313132;">
                {{ rupiah($data['order']->total_price) }}
            </td>
        </tr> --}}
        {{-- @foreach ($data['order']->orderPayments()->whereNot('status', \App\Enums\PaymentStatus::REJECTED()) as $orderPayment)
            <tr style="margin-bottom: 10px;">
    ...
    <footer>
                    ...
                    <td style="text-align: left; padding: 0; width: 50%; vertical-align: top;">
                        ...
                        <ul>
                            ...
                            <li>
                                ...
                                <ul>
                                    <li>Bank: {{ $data['order']->company->companyAccount->bank_name ?? '-' }}</li>
                                    <li>Account No:
                                        {{ $data['order']->company->companyAccount->account_number ?? '-' }}</li>
                                    <li>Account Name:
                                        {{ $data['order']->company->companyAccount->account_name ?? '-' }}</li>
                                </ul>
                            </li>
                            <li>
        ...
        <div style="
            ...
            {{ strtoupper($data['order']->company->companyAccount->name ?? 'our company') }}
        </div>
    </footer>
</body>
